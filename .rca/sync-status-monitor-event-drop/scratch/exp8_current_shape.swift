...
// Experiment 8: Reproduce the failing test's race with the CURRENT bridge shape
// (Task { register }), to see if H1's Race A is observable.
...
actor Bridge {
    ...
    var eventStream: AsyncStream<Int> {
        AsyncStream { continuation in
            let id = UUID()
            Task { self.register(id: id, continuation: continuation) }  // <-- CURRENT
        }
    }
...
actor Monitor {
    ...
    func start() async {
        ...
        task = Task { [weak self] in
            for await ev in s { await self?.apply(ev) }
        }
    }
