...
## 2026-07-31 — Settle AppIntents questions from the compiled `extract.actionsdata`, not from source; and remember App Shortcuts are a *second* configurable surface

LIL-91's intake carried an alarming lead: the widget's configuration row was
reported as labeled **"No Tags"**, a string that appears nowhere in the widget
target, and concluded *"the configuration UI is resolving against AppIntents
metadata that does not belong to the widget's intent."* Cross-extension metadata
contamination would be a serious, hard-to-fix problem. It was refuted in minutes
once the question was asked of the **build artifact** instead of the source.

- **Every appex that vends intents ships a readable metadata bundle:**
  `<Product>.app/PlugIns/<Ext>.appex/Metadata.appintents/extract.actionsdata`.
  It is **plain JSON**, generated by `appintentsmetadataprocessor`, and it is the
  exact catalog the system indexes. It lists `actions` (with each parameter's
  resolved `title/key`), `entities`, `queries`, `enums`, and `autoShortcuts`.
  Reading it answers "what does the system actually think this extension
  declares?" — a question source code can only approximate, because the answer
  depends on what the processor extracted and on which target it ran for.
- **Test contamination structurally: compare identifier namespaces across the
  bundles.** For Lillist the two intent-vending appexes are provably disjoint —
  zero collisions across `actions`/`entities`/`queries`/`enums` (widget:
  `SelectFilterIntent`, `AdvanceTaskStatusFromWidget`, `SmartFilterEntity`;
  shortcuts: nine `*Intent`s, `TaskEntity`, `StatusAppEnum`). Disjoint namespaces
  refute "the wrong metadata is being resolved" without needing a device.
- **`AppShortcutsProvider` creates a second configurable Lillist surface, and
  users do not distinguish it from the widget.** `autoShortcuts[0]` is
  `AddTaskIntent` ("Add Task", `plus` glyph). Its `parameters[2]` is
  `Tags: [String]?`, and **AppIntents renders an unset optional parameter as
  `No <Title>`** — literally **"No Tags"**. That is the entire origin of the
  scary string: a real, correct, non-defective label belonging to a *different*
  Lillist surface. When a bug report quotes a parameter label, first ask *which
  of the app's intent surfaces owns that label* before assuming a mix-up.
- **The `No <Title>` rendering rule is worth internalizing.** Any unset optional
  `@Parameter` displays as "No " + its title, so an intent's parameter titles
  leak into UI strings users will quote back at you. `SelectFilterIntent`'s own
  default renders "No Filter" for exactly this reason — one word away from the
  reported string, which is precisely why the report was so persuasive.
- **Meta-lesson: "explains everything with no residue" is a claim that needs
  auditing, not a flourish.** `DIAGNOSIS.md` asserted it while silently omitting
  the single observation the intake had called its *sharpest distinction*. The
  hypothesis was still correct; the completeness claim was not. When a diagnosis
