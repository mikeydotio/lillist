...
2. **Only the iOS app wires a history consumer.** macOS has no `RemoteChangeReconciler`. The `DiagnosticHistoryObserver` is net-new wiring on **both** platforms and **must use its own** `PersistentHistoryTokenStore` key (NOT the reconciler's `io.mikey.lillist.persistentHistoryToken`) or the two consumers clobber each other's watermark.
