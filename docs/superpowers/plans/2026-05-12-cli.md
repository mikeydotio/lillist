...
- **Plan 4 — Recurrence (landed):** Plan 4 shipped `SeriesStore` (with `create(fromSeedTask:rule:)`, `forkFutureFromInstance(instanceID:)`, `update(id:rule:)`, `delete(id:)`, `list()`), a Codable `RecurrenceRule` value type (calendar + after-completion variants), and `RecurrenceSpawner` (auto-invoked from `TaskStore.transition` on close). The CLI's `--recurrence` flag on `add`/`edit` and the `lillist filters` predicate `recurrence.isRecurring` were originally stubbed in Plan 6 with `LillistError.validationFailed` "not yet implemented" — those stubs **must now be replaced with real calls into `SeriesStore`**. See the post-completion note at the bottom of this plan for the specific tasks to revisit before sign-off if Plan 6 was authored before Plan 4 landed.
- **Plan 5 — Notifications (landed):** Plan 5 shipped `NotificationScheduler` (actor), `NotificationSpecStore`, `addNudge(taskID:fireDate:)`, `addOffset(taskID:anchor:offsetMinutes:)`, snooze handling, the four delivery layers, and `TaskStore.scheduleFollowUp`. The CLI's `lillist nudge` verb's placeholder stub must now be replaced with a real `NotificationSpecStore.add(taskID:kind:.nudge,offsetMinutes:nil,fireDate:)` write. **The CLI process should NOT wire `TaskStore.notificationScheduler`** — the CLI runs offline/batch, never presents notifications, and reconciliation only makes sense from a long-running app. Leave `taskStore.notificationScheduler = nil`; the spec write persists and the next time the macOS / iOS app launches, it reconciles the new spec into a pending request.
...
> **Plan 3 deviations baked in:** Plan 3 renamed the package's namespace enum from `LillistCore` to `LillistCoreInfo` so the module name no longer shadows it. That means **`LillistCoreInfo.version`** (not `LillistCore.version`) — already updated in the snippets below. Plan 3 also keeps the public predicate type named **`Predicate`**, but Foundation now ships a generic `Predicate<repeat each Input>` that collides with it whenever a file imports Foundation (which `swift-argument-parser` does transitively). In CLI code, refer to it as **`LillistCore.Predicate`** wherever the unqualified name would be ambiguous — already updated below. Same precaution for `LillistCore.Tag` if the CLI ever needs to type it (Plan 6 currently doesn't).
...
Expected: prints `0.1.0` (or whatever `LillistCoreInfo.version` is set to).
        ...
        public func toPredicateGroup(now: Date, calendar: Calendar, applyTrashImplicit: Bool = true) throws -> PredicateGroup {
            ...
            if statuses.isEmpty == false {
                let leaves = statuses.map { s in
                    LillistCore.Predicate.leaf(.init(field: .status, op: .is, value: .int(s.rawValue)))
                }
...
> **Plan-5 dependency for `nudge` (Plan 5 has landed):** Replace the placeholder journal entry with a real `NotificationSpec` write:
> ```swift
...
>     kind: .nudge,
>     offsetMinutes: nil,
>     fireDate: parsedFireDate
> )
> ```
> The CLI returns immediately after the write; the running macOS/iOS app reconciles the new spec into a `UNNotificationRequest` when its next mutation event fires (or on launch). Do NOT construct a `NotificationScheduler` from the CLI — see the Plan 5 deviation note at the top of this plan.
...
Plan 6 is complete. The `lillist` CLI verbs are implemented for design Section 6, with `nudge` (Plan 5) and `report-crash` (Plan 9) explicitly stubbed for later plans. The `CLIBridge` namespace is ready for App Intents reuse in Plans 7 and 8.

**Recurrence flags** (`--recurrence` on `add`/`edit`, the `lillist filters` predicate `recurrence.isRecurring`, and any `lillist series` subcommand surface) call directly into `SeriesStore` (Plan 4). If you encounter a stub returning `LillistError.validationFailed("recurrence not yet implemented")` while executing this plan, replace it with a real call into `SeriesStore`:

- `--recurrence "FREQ=DAILY;INTERVAL=1"` (or similar RRULE-subset string parsed into `RecurrenceRule.CalendarRule`) on `add` → after creating the task, call `SeriesStore.create(fromSeedTask: id, rule: rule)`.
- `--recurrence none` on `edit` → fetch the task's `series`, call `SeriesStore.delete(id:)` (or remove the membership, depending on whether the user is editing the seed or an instance).
- `--edit-all-future` on `edit` of a non-seed instance → call `SeriesStore.forkFutureFromInstance(instanceID:)` before applying the field changes.
- `recurrence.isRecurring` filter predicate → compile to `series != nil` against `LillistTask`.

The auto-spawn on close is automatic — `TaskStore.transition` already wires `RecurrenceSpawner` (Plan 4 Task 17), so CLI `lillist close <task>` triggers it without any CLI-side work.
...
- [ ] `lillist report-crash` and `lillist nudge` are clearly stubbed with pointers to the owning plan (9 and 5 respectively).
- [ ] Recurrence flags (`--recurrence`, `--edit-all-future`, `recurrence.isRecurring`) call into `SeriesStore` directly — no stubs remain. (Plan 4 has landed; see post-completion note above.)
