...
> **📍 STATUS — ✅ COMPLETE — merged to `main` (Wave 6; commits `ee19b13`..`f4aa02f`).** Closed cli-2…6. `StoreLocator` left inline (not routed through the resolver); `PurgeCommand` kept on per-resolution `hardDelete`; golden fixtures regenerated from the real renderers. See `docs/superpowers/handoffs/wave-6.md`.
>
> **⚠️ Wave-5 reconciliation (2026-06-05):** Still essentially unaffected. Verified against `main`: Wave 5 (`4dc1f96..0bd7796`) did **not** touch any file this plan modifies — `Resolver.swift`, `WatchHandler.swift`, `RestoreHandler.swift`, and all five destructive commands + `WatchCommand.swift` are unchanged, so every Task 1/3/4/7 code anchor still holds (anchor by **structure**, not the stale in-task line numbers). Wave 5 introduced `GatedPersistenceResolver` (`LillistCore/Sync/`), but this plan does **not** use it: the rewritten commands only *call* `CLIBridge.StoreLocator.openAppGroup()` (API unchanged), and `StoreLocator` still builds `MigrationGate` inline. **Do not route `StoreLocator` through `GatedPersistenceResolver` here** — per the corrected Wave-5 resolver header (commit `20b6dab`), unifying `StoreLocator` through the resolver is an **unscoped optional follow-up owned by no plan**; `extension-persistence-unification` (the FIRST Wave-6 plan) routes only the App-Intents `TaskEntityQuery`, not `StoreLocator`. Wave 5's iOS test churn (deleted/renamed app-target tests, the `IntentSupport` co-compile, `DragDropResolver`) does not touch this plan — it lives entirely in the LillistCore SPM targets (`LillistCoreTests`, `lillistCLITests`), which Wave 5 left intact.
>
