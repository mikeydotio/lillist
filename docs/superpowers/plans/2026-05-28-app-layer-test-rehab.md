...
> **📍 STATUS — ✅ MERGED — Wave 5 (commits `bcc1e57`..`656e353`, 2026-06-05).** Closed ios-2, ios-3, macos-4, ext-6. Introduced the three Wave-6 seams (`GatedPersistenceResolver`, `DragDropResolver`, `TaskListShortcutGate`). iOS + macOS apps build; iOS standalone bundle 29 tests pass; macOS scheme TEST SUCCEEDED; +5 `DragDropResolverTests` pass. The 3 Wave-4 `Lillist-iOSAppHostedTests` entries survived regeneration. Adversarially reviewed (regression: no findings; spec + code-quality `approve_with_notes` — the lone code finding, a `GatedPersistenceResolver` header that overclaimed CLI adoption, was fixed in `656e353`). **Commit grouping deviation**: plan Tasks 5+6 and 7+8+9 each landed as one buildable commit (shared pbxproj regeneration). See `docs/superpowers/handoffs/wave-5.md`.
>
> **⚠️ Wave-4 reconciliation (2026-06-04):** This plan's *direct* source touchpoints survived Wave 4 cleanly — `IntentSupport.swift`, `ShareRootView.swift`, both `applyDrop` bodies, and `FocusedListColumn.swift`/`LillistCommands.swift` are all byte-identical to what the tasks assume, and `GatedPersistenceResolver` wraps **`MigrationGate`** (unchanged), *not* `MigrationCoordinator` (which Wave 4 grew two params + reentrancy guards — irrelevant here). Three things to know:
> - **Task 8 edits a DIFFERENT iOS `project.yml` target than Wave 4 did.** Wave 4 added three sources to the **`Lillist-iOSAppHostedTests`** target (`StoreReconfigureConcurrencyTests.swift`, `MigrationCoordinatorRestoreTests.swift`, `Helpers/FakeUserNotificationCenter.swift`). Task 8 edits the **`Lillist-iOSTests`** sources block (co-compile `IntentSupport.swift`). These are distinct targets, so there's no logical conflict — but **re-Read `Apps/Lillist-iOS/project.yml` before editing and before every `xcodegen generate`**, and confirm the three new AppHostedTests entries are still present after regeneration so you don't clobber them. (Anchor by the `Lillist-iOSTests:` target's sources list, not by line number — the file grew.)
> - **`MigrationJournal` gained `isStale`/`staleThreshold` in Wave 4**, but Task 1's test constructs `MigrationJournal(state: .reconfiguringStore)` and `InMemoryMigrationJournalStore(initial:)` — both the `.reconfiguringStore` state and that initializer are unchanged, so the snippet still compiles as written.
> - **`FakeUserNotificationCenter.add` is now upsert-faithful** (Wave 4). This plan introduces no notification tests, so no action — noted only so a later editor doesn't trip on it.
>
...
- [ ] **Step 1: Add `IntentSupport.swift` to the iOS test bundle sources.** **Re-Read `Apps/Lillist-iOS/project.yml` first** — Wave 4 added three entries to the *separate* `Lillist-iOSAppHostedTests` target (`StoreReconfigureConcurrencyTests.swift`, `MigrationCoordinatorRestoreTests.swift`, `Helpers/FakeUserNotificationCenter.swift`); do **not** touch or drop those. Your edit goes in the **`Lillist-iOSTests`** target's sources block (the standalone `TEST_HOST: ""` bundle that already co-compiles `SharePayload.swift` + `ReportCrashIntent.swift`), after the `ReportCrashIntent.swift` co-compile line (`- path: ../../Extensions/ShortcutsActions/ReportCrashIntent.swift`). Anchor by that `ReportCrashIntent.swift` line, **not** by an absolute line number (the file grew in Wave 4). The `../../Extensions/ShortcutsActions/IntentSupport.swift` path is relative to `Apps/Lillist-iOS/` and matches the `ReportCrashIntent.swift` co-compile path verbatim (same directory) — confirm both files sit in `Extensions/ShortcutsActions/`. After regenerating the pbxproj, confirm the three AppHostedTests entries are still present.
