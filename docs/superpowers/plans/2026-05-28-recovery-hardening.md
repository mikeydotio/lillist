...
>
> **✅ Wave-5 reconciliation (2026-06-05):** Verified clean — Wave 5 (`4dc1f96..0bd7796`) touched none of this plan's targets (`LillistError.swift`, the new `DiskSpaceProbe.swift`, `QuarantineManager.swift`, `MigrationCoordinator.swift`, `SyncMigrationRecoverySheet.swift`). The Wave-4 note below remains the operative anchor guidance (two reentrancy guards in `runMigration` — do NOT add a third; the comment-only step-5 copy-block edit; `MigrationJournal.isStale` is consumed, not re-added).
>
> **⚠️ Wave-4 reconciliation (2026-06-04):** Wave 4 heavily edited the shared `MigrationCoordinator.swift` but left this plan's other targets (`QuarantineManager`, `LillistError`, `SyncMigrationRecoverySheet`) untouched, so most anchors survive. Adapt as follows:
> - **`MigrationCoordinator` is now a Wave-4 hotspot — re-Read `runMigration` and re-anchor by step comments before Task 4.** `runMigration` already OPENS with TWO reentrancy guards (a synchronous `journal.read().isInFlight` throw, then a `@MainActor isMigrating` flag with `defer { isMigrating = false }`) followed by `await breadcrumb(...)`, all before the phase work — **do NOT add a third guard.** Step 6 gained a pre-erase `accountStateProvider` guard; step 8 gained a `restoreSteadyState(...)` call. **Task 4's target — the `// 5. quarantine…` copy block — is byte-identical to my "before" snippet and stays a comment-only edit;** just re-anchor on the step-5 comment, not line numbers.
> - **`MigrationCoordinator.init` grew TWO optional params** (`preferencesStore: PreferencesStore? = nil` after `notificationScheduler:`, and `accountStateProvider: AccountStateProviding? = nil` after `localStoreRowCount:`). Both default nil, so **Task 5's test construction still compiles unchanged** — and omitting `accountStateProvider` (nil ⇒ no account pre-flight) is exactly right for the disk-full path. No edit required; do NOT add the new args to the test.
> - **`MigrationJournal.isStale` + `staleThreshold = 600` already landed in Wave 4**, and the iOS/macOS recovery sheets already gate `recoveryJournal = journal` behind `isStale()`. This plan never added isStale and adds no journal-gating — **no conflict, nothing to delete;** just consume the existing API.
> - **Untouched by Wave 4:** `QuarantineManager.copyStore`/`quarantineStore`/init (Task 3 anchors exact), `LillistError` enum + switch (Task 1 anchors exact), `SyncMigrationRecoverySheet` `detail`/`operationDescription` (Task 8 anchors exact). The whole copy/QuarantineManager chain remains this plan's, as does the new `DiskSpaceProbe` seam.
>
...
> **By Wave 7, `MigrationCoordinator` already carries Wave-4's migration-adjacent-correctness changes** (verified on current `main`): `runMigration` opens with TWO reentrancy guards (`journal.read().isInFlight` throw, then the `@MainActor isMigrating` flag) + an `await breadcrumb(...)`; step 6 has a pre-erase `accountStateProvider` guard; step 8 calls `restoreSteadyState(...)`. **Do NOT add a third reentrancy guard.** Re-Read `runMigration` and anchor by the step comments (`// 5. quarantine the now-closed old store…`) rather than line numbers. The step-5 copy block itself is unchanged from this plan's "before" snippet — this remains a comment-only edit.
