...
> **📍 STATUS — ✅ MERGED to `main` — Wave 4** (commits `5caef1e`..`45b7d7d`; 805 LillistCore tests green, warning-free; iOS + macOS apps build clean). Closed threading-1, persist-4, conc-5, notif-7, persist-1. **Two required deviations from the plan, both correct:** (1) `batchPurge` takes a predicate *format + args* and rebuilds `NSPredicate` inside the `perform` (NSPredicate is non-Sendable under the strict-concurrency source target); (2) `NSBatchDeleteRequest(objectIDs:)` requires a SINGLE entity — the plan's single mixed-entity batch threw `NSInvalidArgumentException` at runtime, so a `CascadeReaper.batchDelete(objectIDs:in:)` was added that groups by entity and deletes **leaf-first** (Attachment → NotificationSpec → JournalEntry → LillistTask), returning the union of executed IDs to merge. `HistoryPruner.sweep()` is wired fire-and-forget into both `bootstrap()`s. Residual #3 (un-cancelled OS-level pending notifications on purge) documented as an acknowledged limitation in `docs/engineering-notes.md`.
>
