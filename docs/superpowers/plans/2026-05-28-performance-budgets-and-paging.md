...
> **📍 STATUS — ✅ COMPLETE — merged to `main` (Wave 6; commits `ae18af5`..`8782757`).** Closed the §761 perf budget + unbounded-fetch findings. `evaluate(group:)`'s `limit` is now a non-optional `Int = 0` (+`offset: Int = 0`); `children(of:)` routes through `childrenFetchRequest` (`fetchBatchSize=100`) + a new `children(of:limit:offset:)` overload, co-landed under `observability-logging`'s signpost wrapper. See `docs/superpowers/handoffs/wave-6.md`.
>
> **⚠️ Wave-5 reconciliation (2026-06-05):** Wave 5 (`4dc1f96..0bd7796`) touched **none** of this plan's four production files (`TaskStore.swift`, `TaskStore+Queries.swift`, `SmartFilterStore.swift`, `NSPredicateCompiler.swift`) — every Task 4–6 code anchor and "before" snippet still matches. Two doc/scaffolding anchors moved: **(1)** Wave 5 appended a **new** last entry to `engineering-notes.md` — `## 2026-05-28 — Crash-reporter redaction is layered, and the canary can't trust PID alone` (~line 2217) — *after* the Wave-4 background-seam entry the note below names, so the true EOF is now that crash-redaction entry; Task 6 re-Reads the tail and appends after it. **(2)** Wave 5 grew the LillistCore suite (`GatedPersistenceResolverTests` + the crash-reporting suites), so Task 7's "649 Swift Testing tests" baseline is stale — assert the suite *passes*, don't pin a fixed count. The Wave-5 `GatedPersistenceResolver`/`ShareRootView`/`IntentSupport`/`DragDropResolver`/`project.yml` work belongs to other Wave-6 plans, not this one.
>
...
Expected: the new `## 2026-06-04 — Performance budgets…` heading appears at the tail (after the Wave-5 `## 2026-05-28 — Crash-reporter redaction is layered…` entry, the current EOF); the heading count incremented by exactly 1 versus before (34 → 35).
...
Expected: the entire suite passes — the existing Swift Testing tests (the baseline grew in Wave 5, which added `GatedPersistenceResolverTests` and crash-reporting suites; assert the suite *passes*, don't pin a fixed count) plus the new XCTest perf suites (`PerfFixtureSmokeTests`, `SmartFilterPerformanceTests`, `SmartFilterPagingTests`, `TaskListFetchPerformanceTests`). Final line resembles `Test Suite 'All tests' passed` / `with 0 failures`. The perf suites add minutes (10k seeding × several fixtures) — that is expected.
