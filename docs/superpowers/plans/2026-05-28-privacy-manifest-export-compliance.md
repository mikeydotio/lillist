...
>
> **⚠️ Wave-5 reconciliation (2026-06-05):** Substance unaffected — all Task 5/6/7/8 anchors still resolve. Two `project.yml` facts to carry: **(1)** Wave 5 inserted `../../Extensions/ShortcutsActions/IntentSupport.swift` into the `Lillist-iOSTests` target's `sources` (~line 145, after the `Tests/UnitTests` anchor at line 130), which shifted the three Wave-4 `Lillist-iOSAppHostedTests` entries the note below places at "lines 199–201" **down +6 to lines 205–207** (they are intact). **Re-Read `Apps/Lillist-iOS/project.yml` before any Task 6 edit and do NOT clobber the AppHostedTests entries OR the new `IntentSupport.swift` co-compile.** **(2)** Both committed pbxprojs were already regenerated in Wave 5 (IntentSupport present; the five Wave-5-deleted tests — `NotesDebounceTests` / `CrashReportingDisclosureGateTests` / `DragDropInteractionTests` / `FocusedShortcutGatingTests` / `SegmentedDetailTabPersistenceTests` — already gone, and the two renamed composition tests current), so Task 6/8's `xcodegen generate` yields a clean privacy-only diff. `Apps/project.yml` (macOS) was untouched by Wave 5.
>
> **⚠️ Wave-4 reconciliation (2026-06-04):** This plan's content (manifests, four Info.plists, the C617.1 `QuarantineManager` + CA92.1 `UserDefaults` rationale, the absence pre-checks) is **unaffected** by Wave 4 — Wave 4 touched none of those files, and `QuarantineManager.swift` / the `Resources/` dirs / the Info.plists are unchanged. The **one** intersection is the iOS xcodegen chain: this plan is the *last* `project.yml` editor and owns the authoritative final `xcodegen generate` + committed pbxproj for `Apps/Lillist-iOS`. Wave 4 added **three** entries to the `Lillist-iOSAppHostedTests` target's `sources` (`StoreReconfigureConcurrencyTests.swift`, `MigrationCoordinatorRestoreTests.swift`, `Helpers/FakeUserNotificationCenter.swift` — now lines 199–201). **Re-Read `Apps/Lillist-iOS/project.yml` before any Task 6 edit and do NOT clobber those entries**; when you regenerate the pbxproj (Task 6 Step 1, Task 8 Step 5), the committed copy must reflect them. The anchors in Tasks 6/7 still resolve: iOS `Resources` `buildPhase: resources` at lines 45–46; `ShareExtension-iOS.sources` excludes ~79–83; `ShortcutsActions.sources` excludes ~102–107; `Lillist-iOSTests` `Tests/UnitTests` at line 130; macOS `Resources` at lines 38–41 — none moved.
>
...
>
> **This is the last `project.yml` editor in both xcodegen chains** (iOS chain #4 and macOS chain #5 in the [index](2026-05-29-foundation-hardening-index.md#shared-file-serial-chains-do-not-parallelize-these)). It owns the final coordinated `xcodegen generate` for each project, so its regenerated pbxprojs are the authoritative committed copies that the `ci-and-build-posture` drift gate validates. Run the regenerate exactly as in Tasks 6/7 and Task 8 Step 5 so the committed pbxprojs reflect every prior wave's source additions, not just this plan's. **Wave 4 added three sources to `Lillist-iOSAppHostedTests` (`StoreReconfigureConcurrencyTests.swift`, `MigrationCoordinatorRestoreTests.swift`, `Helpers/FakeUserNotificationCenter.swift` — now L205–207, intact). Wave 5 did NOT extend that target; it added `IntentSupport.swift` to the separate `Lillist-iOSTests` bundle (L145) and deleted/renamed several `Tests/` files (all under directory-glob `sources` paths, so no explicit anchor here references them). Both committed pbxprojs were already regenerated in Wave 5, so a fresh `xcodegen generate` produces a clean privacy-only diff. Re-Read `project.yml` immediately before each regenerate so the authoritative pbxproj you commit includes every prior wave's entries — do not regenerate against a stale mental model.**
