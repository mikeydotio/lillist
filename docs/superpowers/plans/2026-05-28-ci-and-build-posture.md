...
>
> **⚠️ Wave-5 reconciliation (2026-06-05):** Wave 5 (`4dc1f96..0bd7796`) again moved pbxproj-relevant trees this plan's drift gate (Task 4) polices, so re-verify against current `main`: **(1)** `Apps/Lillist-iOS/project.yml`'s `Lillist-iOSTests` target gained a **fourth** co-compiled extension source — `../../Extensions/ShortcutsActions/IntentSupport.swift` (alongside the pre-existing `SharePayload.swift` + `ReportCrashIntent.swift`); the three Wave-4 `Lillist-iOSAppHostedTests` entries are intact (shifted to ~L205–207). **Re-Read `project.yml` before touching it; do NOT clobber the IntentSupport co-compile or the three app-hosted entries.** **(2)** Wave 5 deleted 5 test files (`DragDropInteractionTests.swift`, `FocusedShortcutGatingTests.swift`, `SegmentedDetailTabPersistenceTests.swift`, `NotesDebounceTests.swift`, `CrashReportingDisclosureGateTests.swift`), renamed 2 (`QuickCaptureFlowTests`→`LillistCoreQuickCaptureCompositionTests`, `ShareExtensionPayloadTests`→`LillistCoreSharePayloadCompositionTests`), and added new ones (`IntentSupportGateTests`, `FocusedShortcutGatingPredicateTests`, `GatedPersistenceResolverTests`, `DragDropResolverTests`). The committed pbxprojs already match all of this (working tree clean, verified live) — so Task 4's drift gate / Step 4 must still come up **clean**; any diff is pre-existing, not this plan's fault. **(3)** This plan references no Wave-5 symbol (`GatedPersistenceResolver`, `DragDropResolver`, `ShareRootView`, `IntentSupport` delegation, `StoreLocator`, `addLinkPreview`) and no test-count arithmetic, so the Wave-5 resolver/delegation refactors invalidate no task body — this is a re-Read-before-edit reminder, not a content change.
>
