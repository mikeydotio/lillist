...
| `TEST_HOST: ""` | setting | `Apps/project.yml:141` | Marks the test bundle host-less so headless CLI builds work without signing |
...
- Test `sources` co-compile individual helper files, not directories, to avoid
  pulling in `AppEnvironment` and the full app graph (`Apps/project.yml:79`).
