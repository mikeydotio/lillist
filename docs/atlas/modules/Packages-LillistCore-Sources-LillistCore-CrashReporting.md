...
a `now` clock closure at `CrashReporter.swift:40`) so tests substitute fakes.
`BreadcrumbBuffer` is an actor whose entry count is capped at
`BreadcrumbBuffer.capacity` (200) inside `record` (`BreadcrumbBuffer.swift:43`).
...
canary files self-delete on read (`CanaryFile.swift:78`) so a poisoned write
